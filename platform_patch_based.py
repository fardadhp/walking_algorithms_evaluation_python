    ...
    def find_node(self):
        ...
        while lyr != []:
            ...
            for f in range(len(lyr)):
                for m in [-1, 0, 1]:
                    for n in [-1, 0, 1]:  # [lyr[f][0]+m,lyr[f][1]+n]
                        nbrs = []
                        for m1 in [-1, 0, 1]:
                            for n1 in [-1, 0, 1]:
...
for r in range(runs):
    ...
    for k in range(n_agents):
        ...
        # agents_list[k] = Agent(k, 19.880, 69.523, destination)
...
# ####### Run ###########
    ...
    #     file.write(str(agents_list[i].xcor))
    #     file.write(" ")
    #     file.write(str(agents_list[i].ycor))
    #     file.write("\n")
    # file.close()
    ...
    # for i in range(1):
        ...
        for j in active_agents:
            # print(agents_list[j].xcor // 1 - 35, agents_list[j].ycor // 1 - 35)
            agents_list[j].pre_node = [agents_list[j].xcor,agents_list[j].ycor]
            # ax.scatter(agents_list[j].xcor, agents_list[j].ycor, color='grey', s=5, marker='.', zorder=2)
            if algorithm == 'COPE0':
    ...
    file.write("\n")
    file.write("# trapped agents = ")
    file.write(str(n_trapped))
    file.write("\n")
    file.write("# nonconverged agents = ")
    if n_steps == 501:
        file.write(str(1))
    else:
        file.write(str(0))
